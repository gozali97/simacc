    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            buatasetBtn.addEventListener('click', () => {
                ...
                const namaAset = document.querySelector('#nama').value;
                const hurufAwal = namaAset.slice(0, 2).toUpperCase();

                for (let i = 0; i < jumlah; i++) {
                    const newCard = document.createElement('div');
                    newCard.className = 'card mb-3';
                    newCard.innerHTML =
                        '<div class="card-header">Detail Aset</div><div class="card-body"><div class="row"><div class="col"><label class="form-label">Kode Detail Aset</label> <input id="kode' +
                        (i + 1) +
                        '" type="text" name="kode[]" class="form-control @error('kode') is-invalid @enderror" placeholder="Kode Aset" required value="' +
                        hurufAwal + pad(i + 1, 3) +
                        '" /> @error('kode') <span class="invalid-feedback" role="alert"> <strong>{{ $message }}</strong> </span> @enderror </div> <div class="col"><label class="form-label">Ruang</label><select id="ruang' +
                        (i + 1) +
