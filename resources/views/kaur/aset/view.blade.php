...
<div class="row p-3">
    ...
    <div class="col-md-12">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="asal" class="form-select @error('asal') is-invalid @enderror"
                                    ...
                                    @foreach ($asal as $a)
                                    <option value="{{ $a->id }}" {{ $a->id == $data->kd_asal ? 'selected' : '' }}>{{
                                        $a->asal_aset }}</option>
                                    @endforeach
                                </select>
                                @error('asal')
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select name="jenis" class="form-select @error('jenis') is-invalid @enderror"
                                    ...
                                    @foreach ($jenis as $j)
                                    <option value="{{ $j->kd_jenis }}" {{ $j->kd_jenis == $data->kd_jenis ? 'selected' :
                                        '' }}>{{ $j->nama_jenis }}</option>
                                    @endforeach
                                </select>
                                @error('jenis')
        ...
        <form action="{{ route('aset.updateDetail', $details->kd_det_aset) }}" method="POST">
            @csrf
            ...
            <div class="card mt-3">
                <div class="row">
                    ...
                    {{-- <div class="col-2"> <button type="submit"
                            class="btn rounded-pill btn-info mt-4">
                            <i class="tf-icons bx bx-pencil" style="margin-right: 5px"></i>Ubah
                        </div> --}}
                </div>
                    ...
                    <div class="row">
                        <div class="col">
                            ...
                            <select id="ruang" name="ruang" class="form-select @error('ruang') is-invalid @enderror"
                                ...
                                @foreach ($ruang as $r)
                                <option value="{{ $r->kd_ruang }}" {{ $r->kd_ruang == $details->kd_ruang ? 'selected' :
                                    '' }}>{{ $r->nama_ruang }}</option>
                                @endforeach
                            </select> @error('ruang') <span class="invalid-feedback" role="alert"> <strong>{{ $message
                        ...
                        <div class="col">
                            ...
                            <select id="kondisi" name="kondisi"
                                ...
                                <option value="{{ $k->id }}" {{ $k->id == $details->kd_kondisi ? 'selected' : '' }}> {{
                                    $k->kondisi_aset }}</option>
                                @endforeach
                            </select>
                            @error('kondisi') <span class="invalid-feedback" role="alert"> <strong>{{ $message
                                    }}</strong> </span> @enderror
                        </div>
                        <div class="col">
                            <div class="row">
                                <div class="col-9"> <label for="formFile" class="form-label">Foto</label>
                                    <input class="form-control @error('gambar') is-invalid @enderror" type="file"
                                        id="gambar" name="gambar" id="formFile"/> @error('gambar')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span> @enderror
                                </div>
